...
import { DEMO_LOGIN_SESSION_KEY, DEMO_LOGIN_TRANSITION_MS, validateDemoCredentials } from "../../lib/demo-login";
import {
...
export function App() {
  const [authenticated, setAuthenticated] = useState(() => window.sessionStorage.getItem(DEMO_LOGIN_SESSION_KEY) === "true");
  ...
  if (!authenticated) return <ShowcaseLogin onSuccess={() => setAuthenticated(true)} />;
...
function ShowcaseLogin({ onSuccess }: { onSuccess: () => void }) {
  ...
  return <main className="demo-login-page">
    ...
    <section className="demo-login-shell" aria-labelledby="demo-login-title">
      <div className="demo-login-intro">
        ...
        <div><span className="demo-login-index">RESEARCH DATABASE / 01</span><h1 id="demo-login-title"><span>中国日化前沿</span><span>靶点与植物化学</span><span>数据库检索平台</span></h1><p>连接小分子化合物、天然产物、分子靶点与科研文献数据。</p></div>
        <div className="demo-login-network" aria-hidden="true"><span>化学结构</span><i /><span>生物活性</span><i /><span>科学文献</span></div>
      </div>
      <div className="demo-login-form-panel">
        <div className="demo-login-form-heading"><span>DATABASE ACCESS</span><h2>登录检索平台</h2></div>
        <form className="demo-login-form" method="post" action="./" onSubmit={submit} noValidate>
          <label htmlFor="pages-login-username">用户名</label>
          <div className="demo-login-input"><input id="pages-login-username" name="username" value={username} onChange={(event) => { setUsername(event.target.value); setError(""); }} autoComplete="username" placeholder="请输入用户名" /></div>
          <label htmlFor="pages-login-password">密码</label>
          <div className="demo-login-input"><input id="pages-login-password" name="password" value={password} onChange={(event) => { setPassword(event.target.value); setError(""); }} autoComplete="current-password" type="password" placeholder="请输入密码" /></div>
...
function DemoLoginTransition() {
  return <main className="demo-login-page demo-login-transition-page">
    <div className="demo-login-grid" aria-hidden="true" />
    <section className="demo-login-transition-card" role="status" aria-live="polite" aria-busy="true">
      <div className="demo-login-transition-mark" aria-hidden="true"><span /><span /><i /></div>
